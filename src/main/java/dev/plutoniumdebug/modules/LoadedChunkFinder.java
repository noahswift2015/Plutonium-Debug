...
public final class LoadedChunkFinder extends Module {
    ...
    private final Setting<Integer> radius = sgGeneral.add(new IntSetting.Builder().name("radius").description("Horizontal scan radius in loaded blocks.").defaultValue(32).min(8).sliderMax(96).build());
    private final Setting<Boolean> crops = sgGeneral.add(new BoolSetting.Builder().name("mature-crops").description("Report mature crop and wart blocks.").defaultValue(true).build());
    private final Setting<Boolean> kelp = sgGeneral.add(new BoolSetting.Builder().name("kelp").description("Report visible kelp and cave vines.").defaultValue(true).build());
    private BlockPos lastReport;
    private int cooldown;

    public LoadedChunkFinder() { super(PlutoniumDebug.CATEGORY, "sus-chunk-finder", "Finds observable plant activity in already-loaded chunks."); }
